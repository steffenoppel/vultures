...
INPUT <- list(y.terrvis = y.terrvis,
              ...
              phase=c(1,2,2,1,2,1,1,1,1,2,1,2,1,1),  ## best matches observed variation in pop size
...
cat("
  model {
    ...
    for (nyRpterr in 1:nprim.terrvis){
      ...
      rand.phi.terrvis[nyRpterr] ~ dnorm(0, tau.phi.terrvis)  ## include the decline in count data as prior mean for random effect
    }
